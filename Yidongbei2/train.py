...
def get_parser():
    ...
    parser.add_argument('--train_path', type=str, default=r'Yidongbei2/Data_version/training/total_train.csv',
                        help='Step3中生成的保存训练集的csv文件地址')
    parser.add_argument('--target_path', type=str, default=r'Yidongbei2/Data_version/training/img_unlabeled',
...
def train(source_loader, target_loader, model, optimizer, lr_scheduler, args):
    ...
    for e in range(1, args.n_epoch + 1):
        ...
        for i in range(n_batch):
            ...
            #计算源域损失
            """
            source_task_clf:原分类任务的结果,32
            source_domain_clf:目标域 和源域的判别结果,32x2
            s_domain_output:对应类判别器输出的分类概率,32x2
            s_feat2:特征提取层输出的特征;32x512
            """
            ...
            #计算目标域损失
            """
            taget_label:原分类任务的结果,32，无用
            target_domain_clf:目标域 和源域的判别结果,32x2
            t_domain_output:对应类判别器输出的分类概率,32x2
            Tfeat2:特征提取层输出的特征;32x512
            """
            ...
            # 计算目标域每个分类器的域判别损失（条件分布）
            err_domain_target = domain_criterion(t_domain_output, torch.ones(t_domain_output.shape[0],dtype=torch.long).to('cuda'))

            local_loss = 0.01 * (err_domain_target + err_domain_source)             # 条件分布损失
            gloable_loss = 0.05 * (err_domain_clf_target + err_domain_clf_source)   # 边缘分布损失
            train_pred = torch.max(source_task_clf, 1)[1]
            train_correct += torch.sum(train_pred == source_label)                  # 源域预测准确度
            domain_pred = torch.max(target_domain_clf, 1)[1]
            domain_correct += torch.sum(domain_pred == torch.ones(t_domain_output.shape[0],dtype=torch.long).to('cuda'))  # 目标域域判别准确度
            lossMMD2 = MMDLoss()
            mloss = 3 * lossMMD2(Sfeat2, Tfeat2)           # MMD损失

            tmpd_c += 2 * (1 - 2 * (local_loss))
            tmpd_c /= args.num_class                       # 一个batch内的条件分布损失累加和

            d_c = d_c + tmpd_c.cpu().item()
            d_m = d_m + 2 * (1 - 2 * gloable_loss.cpu().item())
            join_loss = (1 - MU) * gloable_loss + MU * local_loss   # MU为动态的调节系数
            loss = err_task_source + join_loss             #总损失
...
def main():
    ...
    optimizer = get_optimizer(model, args)        # 初始化优化器
