...
# 测试集 2000；有标签训练集 8 * 500；无标签训练集 2000
if __name__=='__main__':
    #原始训练集中没有标签的数据集地址
    unlabeled_data_files = 'Row_data\\training\\unlabeled'       # 无标签的原始训练数据
    #生成的没标签的训练集图片保存的文件夹
    unlabeled_save_root = 'Data_version\\training\\img_unlabeled'

    generate_unlabel_img(unlabeled_data_files, unlabeled_save_root)
    # Step1生成的训练集的csv文件地址
    train_csv_file = 'trian.csv'                                  # 有标签的原始训练数据
    # 生成的有标签的训练集图片的保存文件夹
    train_save_root = 'YData_version\\training\\img_labeled'
